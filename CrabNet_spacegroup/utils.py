...
from CrabNet_spacegroup.composition import generate_features, _element_composition
...
class EDMDataset(Dataset):
    ...
    def __init__(self, dataset, n_comp):
        ...
        self.space_group = np.array(self.data[3])

        self.shape = [(self.X.shape), (self.y.shape), (self.formula.shape), (self.space_group.shape)]
...
def get_edm(path, elem_prop='mat2vec', n_elements='infer',
    ...
    y = df[target_col].values.astype(data_type_np)
    formula = df['formula'].values
    space_group = df['spacegroup'].values.astype(data_type_np)
    ...
    return out, y, formula, space_group
...
class EDM_CsvLoader():
    ...
    def __init__(self, csv_data, batch_size=64,
        ...
        self.main_data = list(get_edm(self.csv_data, elem_prop='mat2vec',
                                      ...
                                      scale=scale, target_col=target_col))
