...
# 定义embedding模型
class SpaceGroupEmbeder(nn.Module):
    def __init__(self, input_size=1, num_heads=4, compute_device=None):
        super().__init__()
        self.fc_layer = nn.Linear(input_size, 512, device=compute_device, dtype=torch.float)
        self.self_attention = nn.MultiheadAttention(512, num_heads, device=compute_device, dtype=torch.float)

    def forward(self, x):
        x = self.fc_layer(x)
        x = torch.unsqueeze(x, 1)
        x = x.permute(1, 0, 2)  # change to (seq_len, batch_size, embedding_size)
        x, _ = self.self_attention(x, x, x)  # use self-attention
        x = x.permute(1, 0, 2)  # change back to (batch_size, seq_len, embedding_size)
        x = torch.mean(x, dim=1, keepdim=True)  # average over sequence length to get final feature vector
        return x
...
class OutEncoder(nn.Module):
    ...
    def forward(self, src, output, space_group):
        mask = (src == 0).unsqueeze(-1).repeat(1, 1, self.d_model)
        output = output.masked_fill(mask, 0)
        output = torch.sum(output, dim=1, keepdim=True)
        space_group = torch.reshape(space_group, (space_group.shape[0], 1))
        space_group = self.embed(space_group)
        space_group = space_group * 2 ** self.emb_scaler
        x = torch.cat([output, space_group], dim=1)
        x = x.transpose(0, 1)
...
class CrabNet(nn.Module):
    def __init__(self,
        ...
        self.encoder = Encoder(d_model=self.d_model,
                               ...
                               heads=self.heads,
                               compute_device=self.compute_device)
        self.output_encoder = OutEncoder(d_model=self.d_model,
                                         N=self.N,
                                         heads=self.heads,
                                         compute_device=self.compute_device)
    ...
    def forward(self, src, frac, space_group):
        output = self.encoder(src, frac)
        output = self.output_encoder(src, output, space_group)
        space_group = torch.reshape(space_group, (space_group.shape[0], 1))
        # average the "element contribution" at the end
        # mask so you only average "elements"
        mask = (torch.cat([space_group, space_group], dim=1) == 0).unsqueeze(-1).repeat(1, 1, self.out_dims)
